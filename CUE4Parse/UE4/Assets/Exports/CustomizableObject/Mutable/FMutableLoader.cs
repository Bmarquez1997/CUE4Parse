...
public class FMutableLoader
{
    ...
    private Dictionary<uint, FMutableStreamableBlock> ModelStreamables =>
        ModelStreamableData.StreamingData.ModelStreamables;
    ...
    public FMutableLoader(UCustomizableObject customizableObject)
    {
        if (!customizableObject.Private.TryLoad(out UCustomizableObjectPrivate coPrivate) 
            || !coPrivate.ModelStreamableData.TryLoad(out UModelStreamableData coStreamableData))
    ...
    public List<EOpType> ReadByteCode()
    {
        ...
        var bytecodeReader = new FByteArchive("Mutable ByteCode", CustomizableObject.Model.Program.ByteCode);
        foreach (var address in CustomizableObject.Model.Program.OpAddress)
        {
