...
public class UCustomizableObject : UObject
{
    ...
    {
        var mutArchive = new FByteArchive("Mutable ByteCode", Model.Program.ByteCode);
        var s = new List<EOpType>();

        foreach (var op in Model.Program.OpAddress)
        {
            mutArchive.Position = op;

            var opType = mutArchive.Read<EOpType>();

            if (opType == EOpType.IN_ADDIMAGE)
            {
                mutArchive.Position -= 2;

                var instance = mutArchive.Read<uint>();
                var value = mutArchive.Read<uint>();

                var id = mutArchive.Read<uint>();
                var externalId = mutArchive.Read<uint>();

                var name = mutArchive.Read<int>();
                var relevantParametersListIndex = mutArchive.Read<int>();
            }
            else if (opType == EOpType.ME_FORMAT)
            {
