...
namespace CUE4Parse.UE4.Assets.Exports.StaticMesh
{
    ...
    public class FSparseDistanceFieldMip
    {
        ...
        public FSparseDistanceFieldMip(FArchive Ar)
        {
            ...
            if (Ar.Game is >= EGame.GAME_UE5_4 and not EGame.GAME_Fortnite_S27)
            {
    ...
    public class FDistanceFieldVolumeData5
    {
        ...
        {
            LocalSpaceMeshBounds = Ar.Game is >= EGame.GAME_UE5_4 and not EGame.GAME_Fortnite_S27 ? new FBox(Ar.Read<FVector>(), Ar.Read<FVector>(), Ar.Read<byte>()) : new FBox(Ar);
